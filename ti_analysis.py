...
_RE_BASE_FILE = re.compile(r"^64pp_w(\d+)\.out$")   # e.g. 64pp_w7.out only
...
def parse_file(filepath: str) -> tuple[list[np.ndarray], np.ndarray]:
    ...
    Only text after the '4.  RESULTS' header is examined.
    Lines containing '*' are skipped.
    MBAR blocks with fewer than N_LAMBDA entries are discarded.
    """
