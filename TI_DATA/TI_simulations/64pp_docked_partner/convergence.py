...
-----
  ...
  2. Run:   python mbar_convergence_standalone.pyfbhd
...
_RE_BASE_FILE = re.compile(r"^64pp_docked_partner_w(\d+)\.out$")
...
def load_windows() -> dict[int, np.ndarray]:
    all_out = sorted(glob.glob("64pp_docked_partner_w*.out"))
    files   = [f for f in all_out if _RE_BASE_FILE.match(os.path.basename(f))]

    if not files:
        sys.exit("No window files found matching 64pp_docked_partner_w*.out")
